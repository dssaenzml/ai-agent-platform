...
class MockQueryClassifier:
    ...
    def classify(self, query):
        ...
        if any(
            word in query_lower for word in ["search", "weather", "news", "current"]
        ):
            return "web_search"
        elif any(
            phrase in query_lower
            for phrase in ["generate an image", "image of", "sunset over mountains"]
        ):
            return "image_generation"
        elif any(
            word in query_lower
            for word in ["create", "generate", "document", "presentation"]
        ):
