...
class TestKnowledgeBaseManager:
    ...
    def test_similarity_search_with_score(self, kbm):
        ...
        mock_results = [
            (Mock(page_content="High score content", metadata={"source": "high.pdf"}), 0.9),
            (Mock(page_content="Low score content", metadata={"source": "low.pdf"}), 0.3)
        ]
        kbm.vector_store.similarity_search_with_score.return_value = mock_results
        
        results = kbm.similarity_search_with_score("test query", k=2, score_threshold=0.5)
    ...
    def test_search_different_k_values(self, kbm, k_value):
        ...
        mock_documents = [Mock(page_content=f"Content {i}", metadata={"source": f"doc{i}.pdf"}) 
        ...
        kbm.vector_store.similarity_search.assert_called_once_with("test query", k=k_value) 