...
from langchain_qdrant import (
    RetrievalMode,
    QdrantVectorStore as VectorStore
    )
...
class KnowledgeBaseManager:
    ...
    def __init__(
        ...
        self.collection_name = (
            f"{ai_agent_app_name}"
            f"-CharChunkSize-{config.child_chunk_size}"
            )
        ...
        self.docs_directory = os.path.join(
            '.', 'app', 'docs', ai_agent_app_name.lower(),
            )
    ...
    def upload_blob(self, file_path, content_type):
        ...
        blob_url = f"{self.blob_account_url}/" + \
            f"{self.blob_container_name}/" + \
            f"{self.ai_agent_app_name.lower()}/public_docs/" + blob_name
    ...
    def upload_user_blob(
        ...
        logger.info(f"Processing file: {blob_name}")
        blob_url = f"{self.blob_account_url}/{self.blob_container_name}/" + \
            f"{blob_name}"
        blob_client = BlobClient.from_blob_url(
            blob_url=blob_url, 
            credential=credential, 
    ...
    async def process_base_knowledge(self):
        ...
        pptx_files = [os.path.join(root, name)
                        for root, _, files in os.walk(self.docs_directory)
                        for name in files
                        if name.endswith('.ppt') or name.endswith('.pptx')]
        docxs_files = [os.path.join(root, name)
                        for root, _, files in os.walk(self.docs_directory)
                        for name in files
                        if name.endswith('.doc') or name.endswith('.docx')]
        xlsx_files = [os.path.join(root, name)
                        for root, _, files in os.walk(self.docs_directory)
                        for name in files
        ...
        # Filter empty docs
        vector_db_documents = [
            v for v in vector_db_documents
            if v.page_content != ""
            ]
    ...
    async def process_docs(self, docs, user_doc=False,):
        ...
        for i, doc in enumerate(docs):
            ...
            if user_doc:
                file_title = (
                    f"On user's shared file titled as: '{filename}', "
                    )
            else:
                file_title = (
                    f"On the file titled as: '{filename}', "
                    )
            ...
            if (doc.metadata['category'] == "Table") and \
                ...
                doc.metadata["original_page_content"] = doc.metadata[
                    "text_as_html"
                    ]
