...
from ..chain.hallucination_grader import (
...
async def simple_rag_web_img_query_router(
    ...
    if grade == "no":
        ...
        if query_type == "rag_query":
            ...
            logger.info(
                "---DECISION: QUESTION REQUIRES IMAGE GENERATION---"
            )
...
async def simple_rag_web_img_pdf_query_router(
    ...
    if grade == "no":
        ...
        if query_type == "rag_query":
            ...
            logger.info(
                "---DECISION: QUESTION REQUIRES PDF GENERATION---"
            )
...
def rag_router(state):
    ...
    if len(doc_ids) > 0:
        logger.info(
            "---DECISION: RETRIEVE INDIVIDUAL DOCUMENT---"
        )
        return "doc retrieval"
    else:
        logger.info(
            "---DECISION: RETRIEVE PUBLIC DOCUMENTS---"
        )
...
def decide_to_search_web(state):
    ...
    if web_search:
        ...
        logger.info(
            "---DECISION: TRANSFORM QUERY FOR WEB SEARCH---"
        )
