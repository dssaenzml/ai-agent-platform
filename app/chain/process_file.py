...
async def _process_file(
    ...
    logger.info(
        f"Processing file: {request.filename}... {0.0}% complete"
        )
    ...
    logger.info(
        f"Processing file: {request.filename}... {10.0}% complete"
        )
    await adispatch_custom_event(
        "doc_processing",
        {
    ...
    try:
        ...
        # Notify user the doc processing progress
        logger.info(
            f"Processing file: {request.filename}... {15.0}% complete"
            )
        await adispatch_custom_event(
            "doc_processing",
            {
        ...
        # Notify user the doc processing progress
        logger.info(
            f"Processing file: {request.filename}... {25.0}% complete"
            )
        await adispatch_custom_event(
            "doc_processing",
            {
        ...
        if file_ext in supported_llm_image_types:
            logger.info(
                f"Processing file: {request.filename}... {100.0}% complete"
                )
            if user_doc:
                await adispatch_custom_event(
                    "doc_processing",
                ...
                logger.info(
                    f"File {filename} processed successfully. URL: {blob_url}"
                    )
            ...
            raise HTTPException(
                ...
                detail=
                "Base64 file is not "
                f"{list(supported_mime_types.values())}",
            )
        
        # Notify user the doc processing progress
        logger.info(
            f"Processing file: {request.filename}... {30.0}% complete"
            )
        await adispatch_custom_event(
            "doc_processing",
            {
        ...
        for doc in docs:
            ...
            doc.metadata["upload_timestamp"] = datetime.now(
                tzinfo
                ).isoformat()
        ...
        # Notify user the doc processing progress
        logger.info(
            f"Processing file: {request.filename}... {75.0}% complete"
            )
        await adispatch_custom_event(
            "doc_processing",
            {
        ...
        # Notify user the doc processing progress
        logger.info(
            f"Processing file: {request.filename}... {80.0}% complete"
            )
        await adispatch_custom_event(
            "doc_processing",
            {
        ...
        # Log the error and raise an exception
        logger.error(
            f"Error processing file {request.filename}: {error_message}"
            )
        raise HTTPException(
                status_code=400,
                detail=error_message,
