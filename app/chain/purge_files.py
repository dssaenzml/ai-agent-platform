...
def _delete_blob(
    ...
    if user_doc:
        if doc_id:
            prefix_main = f"{kbm.ai_agent_app_name.lower()}/" + \
                "users_docs/" + f"{user_id}/" + f"{doc_id}"
        else:
            prefix_main = f"{kbm.ai_agent_app_name.lower()}/" + \
                "users_docs/" + f"{user_id}"
    else:
        prefix_main = f"{kbm.ai_agent_app_name.lower()}/" + \
            "public_docs/Additional Documents/" + filename
    
    all_blobs = list(
        container_client.list_blob_names(name_starts_with=prefix_main)
        )
    if not all_blobs:
        if user_doc:
            return  {
...
async def _purge_files(
    ...
    if user_doc:
        ...
        logger.info(
            f"Purging user's {user_id} files... {0.0}% complete"
            )
    else:
        ...
        # Notify user the purge progress
        logger.info(
            f"Purging file: {filename}... {0.0}% complete"
            )
    ...
    try:
        if user_doc:
            if doc_id:
                ...
                # Notify user the purge progress
                logger.info(
                    f"Purging file: {user_id}... {50.0}% complete"
                    )
                await adispatch_custom_event(
                    "doc_processing",
                    {
                ...
                # Notify user the purge progress
                logger.info(
                    f"Purging file: {user_id}... {100.0}% complete"
                    )
                await adispatch_custom_event(
                    "doc_processing",
                    {
            ...
            # Notify user the purge progress
            logger.info(
                f"Purging file: {filename}... {50.0}% complete"
                )
            await adispatch_custom_event(
                "doc_processing",
                {
            ...
            # Notify user the purge progress
            logger.info(
                f"Purging file: {filename}... {100.0}% complete"
                )
            await adispatch_custom_event(
                "doc_processing",
                {
